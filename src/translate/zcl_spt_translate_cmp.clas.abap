...
*
*----------------------------------------------------------------------*
CLASS zcl_spt_translate_cmp DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ts_object_list,
        ...
        ref_class TYPE REF TO zcl_spt_translate_cmp,
...
CLASS zcl_spt_translate_cmp IMPLEMENTATION.
...
* Leo todas las clases que heredan de la clase principal de componentes.
    SELECT * FROM vseoextend INTO TABLE lt_classlist
      WHERE refclsname = 'ZCL_SPT_TRANSLATE_CMP'
      AND version = '1'.
